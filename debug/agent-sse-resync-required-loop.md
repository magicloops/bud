...
- A later mobile log sample shows the same stale cursor class, but with a different client behavior:
  - `16:48:24.655` `GET /agent/stream?after=01KSGRK8ZF986CE4VTJSM1H0HR` returns `agent.resync_required`.
  - `16:48:25.730` and `16:48:25.732` the client starts `GET /messages?limit=100` and `GET /thread`.
  - `16:48:25.907` the client starts another `GET /agent/stream?after=01KSGRK8ZF986CE4VTJSM1H0HR` with the exact same stale cursor.
  - `16:48:25.908` the client starts `GET /agent/state`, one millisecond after the stale stream reconnect.
  - `16:48:25.917` the second stale stream also returns `agent.resync_required`.
- This is not the same as the browser-native EventSource retry loop. The mobile client appears to be doing a REST refresh, but it is also opening a new stream with the known-invalid cursor before `/agent/state` can return a fresh `stream_cursor`.
- The strongest mobile-specific interpretation is a recovery ordering race: `agent.resync_required` triggers several refresh actions, but stream reattach is not gated on the `/agent/state` result.
- The service is behaving as designed in both mobile attach attempts because the supplied `after` cursor is still outside the bounded resume window.
...
### 6. Strong for mobile: resync recovery races stream reattach against `/agent/state`
The mobile client receives or infers a resync path and starts canonical refresh requests, but another stream attach still uses the stale cursor before the state refresh completes.

Why it fits:
- the second mobile stream request uses the exact same `after=01KSGRK8ZF986CE4VTJSM1H0HR`
- that stream starts before the observed `/agent/state` request can complete
- the client also fetches `/messages`, `/thread`, `/web-view`, and `/agent/state`, so it is not simply stuck in native transport retry

Expected mobile fix direction:
- close the failed stream
- immediately mark the provided cursor as invalid
- suppress all stream reattach attempts while the resync refresh is in flight
- fetch `/messages` and `/agent/state`
- reconnect only after `/agent/state.stream_cursor` is available
- if the product intentionally attaches before state is available, attach with no cursor as live-only rather than with the known-invalid cursor
