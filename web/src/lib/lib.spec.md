...
- Consumes ds4's catalog-backed `ds4_responses_reasoning_effort` metadata so
  the normal reasoning selector can show `Fast` and `Thinking` without a
  provider-specific browser request field
...
passes non-tag strings through. Rendered by the thread panel's footer build
tag and logged at boot in `main.tsx`.
