...
- `/$budId/$threadId` - Existing thread conversation using the same shared shell plus `/messages` + `/agent/state` bootstrap, bounded-resume agent SSE, persisted selector changes, `client_id`-first message reconciliation, and a user-clicked file viewer pane
