...
- `/$budId/$threadId` - Existing thread conversation with `/messages` + `/agent/state` bootstrap, safe `/terminal/state` bootstrap, bounded-resume agent SSE, structured browser terminal send, and `client_id`-first message reconciliation
...
| `@/lib/thread-terminal-controller` | Browser terminal transport boundary for the thread route |
