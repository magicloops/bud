...
2. **Tool** - What tool was called (`terminal.exec`, `terminal.send`, etc.)
...
**Usage**:
```typescript
const ToolRenderer = getToolContentRenderer('terminal.exec')
if (ToolRenderer) {
...
Tool-specific renderers for the revised terminal contract. Shows exec/send/observe calls with concise summaries, and now surfaces evidence-based `terminal.send` state so developers can distinguish ambiguous sends from settled interactive waits.
