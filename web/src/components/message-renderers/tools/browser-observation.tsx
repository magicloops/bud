...
export function BrowserObservationContent({ payload }: ToolContentRendererProps) {
  ...
  const hasNodes = Array.isArray(observation?.nodes)
  const text = typeof observation?.text === 'string' ? observation.text
    : expanded && hasNodes ? JSON.stringify(observation?.nodes, null, 2) : null
  return <div className="space-y-2 text-sm">
    ...
    {(path || text || hasNodes) && <button className="text-primary underline" onClick={() => setExpanded(value => !value)}>{expanded ? 'Hide observation' : 'Show observation'}</button>}
