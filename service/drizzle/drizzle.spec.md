...
Contains Drizzle Kit config plus the checked-in migration history used for staging. Local development remains schema-first off `src/db/schema.ts` via `db:push`, while the staged chain now also carries the legacy-run schema removal and push-notification schema catch-up.
