...
### `0039_bud_browser.sql`

Squashed Bud-owned browser schema (Phase 3q): creates `browser_handoff`,
`browser_resource` and `browser_session` in their final Phase 3k/3p shape,
adds nullable `agent_invocation.work_duration_ms` / `work_started_at`, then the
composite owner FKs (session→thread, session→resource, handoff→session,
handoff→invocation, resource→bud) and all partial/lookup indexes. Generated by
`drizzle-kit generate` from the `0038` snapshot against `schema.ts`; it replaces
the earlier eight-migration chain (`0039_tiny_loners` … `0046_tired_johnny_blaze`)
that never deployed. Validated by migrating a fresh database from `0000` and
diffing against the `db:push` schema, and by the isolated-schema repository and
timing tests. Deploy before starting the updated service. See
[Phase 3q](../../../plan/bud-owned-browser/phase-3q-migration-squash.md).

### `0040_browser_claim_retirement.sql`

Custom (hand-written) migration created with `drizzle-kit generate --custom`.
Adds the `retire_bud_browser_claim()` plpgsql function and the
`browser_claim_retirement` trigger on `bud`, which retires the active
`browser_resource` whenever `created_by_user_id` or `device_secret` changes,
including same-owner reclaims. Drizzle cannot express triggers, so this file is
