...
type Dependencies = {
  proposals?: Pick<AutomationProposals, "get" | "list" | "decide" | "cancel">;
...
export async function registerAutomationProposalRoutes(server: FastifyInstance, dependencies: Dependencies = {}) {
  const proposals = dependencies.proposals ?? new AutomationProposals();
  ...
  await server.register(async app => {
    ...
    app.setErrorHandler((error, request, reply) => {
      ...
      request.log.warn({ request_id: request.id, status_code: status }, "Automation review request failed");
      return reply.code(status).send({ error: status === 413 ? "payload_too_large" : "automation_proposal_unavailable" });
    });
    app.get<{ Querystring: Record<string, unknown> }>("/api/automations/proposals", async request => {
    ...
    app.get<{ Params: { id: string } }>("/api/automations/proposals/:id", async request =>
      proposals.get(owners.get(request)!, request.params.id));
    app.post<{ Params: { id: string } }>("/api/automations/proposals/:id/decision", { bodyLimit: 4096 }, async request => {
      ...
      if (input.decision === "approve" && !dependencies.approvalEnabled)
        throw new DataRequestError(503, "automation_proposal_approval_unavailable", "Automation review approval is not available yet");
      return proposals.decide(owner, request.params.id, input);
    });
    app.post<{ Params: { id: string } }>("/api/automations/proposals/:id/cancel", { bodyLimit: 4096 }, async request =>
