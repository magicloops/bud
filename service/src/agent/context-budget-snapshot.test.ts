...
test("buildContextBudgetSnapshot prefers provider usage and includes output tokens", () => {
  ...
  assertAvailable(snapshot);
  assert.equal(snapshot.estimated_input_tokens, 1_250);
  assert.equal(snapshot.remaining_context_tokens, 61_750);
  assert.equal(snapshot.basis, "provider_usage_plus_delta");
