...
## Unified terminal input, readiness, and open-command facts

`terminal.send` is the single input tool (plan/terminal-input-unification.md;
`terminal.run` is retired). The executor always dispatches `await:"auto"`
and the daemon decides from terminal state: a submitted line at a shell
prompt awaits the command boundary (result `kind:"command"` with the real
exit code / duration / output), anything into a running program awaits
settle (`kind:"interaction_ack"` with the screen delta). Nothing is refused
while a command is open — input goes wherever the foreground is. The daemon
gates input into an open program on readiness (painted + quiet, capped) and
reports `gated_ms` / `program_ready`; a launch resolves `status:
"interactive"` only once the program is ready (`ready` / `painted` facts).
Without a daemon outcome (service timeout / user interrupt) a submitted line
is reported `still_running` when a command is known open or the session was
...
**Responsibilities**:
- define the normalized `terminal.send`, `terminal.observe`, `terminal.wait`, `web_view.*`, and `ask_user_questions` directive unions used across the split agent seams
...
- serialize `terminal.send` as exactly one model-facing gesture: `text` (+ optional `submit`) or `key`
...
**Current Coverage**:
- terminal tool args serialize the 0.3 model-facing shapes (`terminal.send { text, submit? | key }`, `terminal.observe { view?, lines? }`)
- terminal directives map to canonical provider tool names (`terminal_send`, `terminal_observe`, `terminal_wait`)
...
- normalize historical tool rows into the current vocabulary: legacy `terminal.interrupt` replays as `terminal_send` with `key: "ctrl+c"`, retired `terminal.run` rows and legacy `raw_text`/`command` send rows replay as `terminal_send { text }`; retired wait/readiness fields on old rows are dropped
...
- `tool-definitions.ts` owns the canonical `terminal_send`, `terminal_observe`, `web_view_open`, `web_view_close`, `web_view_list`, and `ask_user_questions` JSON Schema definitions passed to providers, the Bud-offline tool-catalog resolver, plus the normal agent-turn tool-schema token estimate; the `terminal_send` description directs interrupts/exits through `key` gestures (`ctrl+c`) or the program's own quit command — typed shell commands go to the foreground program while one is running

**System Prompt Highlights**:
- the terminal surface: `terminal.send` for everything typed (shell commands with real exit codes at a prompt, text/keys inside programs), `terminal.wait` for waiting, `terminal.observe` for looking at the screen
...
|------|------------|-------------|
| `terminal_send` | `text?`, `submit?`, `key?` (exactly one of `text`/`key`) | The single input tool. Dispatched with `await:"auto"`: at a shell prompt the text runs as a command (`kind:"command"`, real `exit_code`/`duration_ms`/output; `status:"interactive"` + `ready`/`painted` when it launched a program); inside a program the input is gated on readiness and returns the settled screen delta (`kind:"interaction_ack"`, `gated_ms`/`program_ready`) |
...
| `terminal_wait` | none | Knobless: park the turn until the terminal needs attention (command finished, prompt returned, unseen output went quiet — `stalled` — or the screen stayed static for the daemon's 10s cap — `no_activity`), then return the delta; the model configures nothing |
...
When the current agent environment is `bud_offline`, provider calls use a Bud-specific denylist that removes terminal (`terminal_send`, `terminal_observe`, `terminal_wait`) and web-view tools while keeping service-level tools, including `ask_user_questions`, available by default.
...
- A submitted `terminal.send` line at a shell prompt is the deterministic command path: dispatched with `await: "auto"` (resolved by the daemon to the command boundary), the executor awaits `command_finished`, then slices the command's output from the durable store by byte range (`TerminalSessionManager.getCommandOutput`, 64 KiB tail-keeping cap). A failing command (non-zero exit) is a NORMAL tool result carrying `exit_code`; only transport/session problems become structured errors.
- Reattach-amnesia override (debug/terminal-send-codex-endgame-misclassification.md): before dispatch the executor resolves the session's open command from durable `terminal_command` rows; when one is open it sends `await: "settled"` explicitly instead of `"auto"` — the daemon's per-attachment facts forget open commands across reattaches and would misroute input meant for a long-lived inline TUI into the command/sentinel path. Safe on stale rows: sentinel commands only get complete rows, so a stale-open row implies a genuine-OSC133 shell where settled awaits still resolve `command_finished` with real exit codes. Effective for every deployed daemon version.
- If the two-minute awaited-send budget expires without an outcome, a submitted line at a shell returns a still-running report (`status: "still_running"` plus observe guidance) — never a fabricated failure. The report attaches a capped current-screen observation (`view: "screen"`) so a budget burn is never blind — a misrouted send shows its mangled input on the FIRST timeout. Still-running (and interrupted) reports recover the dispatched `command_id` via `TerminalSessionManager.getLatestCommandForSession(sessionId)`, but only while that latest command row is unfinished — a finished latest row means the run's `command_started` event was lost and `command_id` stays `null` rather than mislabeling an older command.
...
- historical persisted `terminal.run` rows and `terminal.send` rows with `command`/`raw_text` replay as `terminal_send { text }`, and `terminal.interrupt` rows replay as `terminal_send` with `key: "ctrl+c"`, so old transcripts round-trip into the current provider/tool vocabulary.
...
- a submitted `terminal.send` line maps the daemon's `input_absorbed` outcome (text typed on an OSC 133 shell but no command started — the shell ran nothing) to `status: "input_absorbed"` with observe guidance; nothing is pending.
- `terminal.wait {}` is the waiting primitive (plan/terminal-wait-async-wakeup.md; knobless per debug/terminal-wait-codex-question-stall.md — the retired `until` knob mis-chose the command boundary during inline-TUI sessions): one awaited `terminal_observe` (`await: "settled"`, `quiet_ms: TERMINAL_WAIT_STALL_QUIET_MS` = 1500) that blocks up to the 30-minute `TERMINAL_WAIT_TIMEOUT_MS`, then returns `kind: "wait"` with `outcome` (`settled | stalled | no_activity | command_finished | prompt_ready | idle (pre-knobless daemons) | closed | timeout | interrupted | superseded`), `waited_ms`, the tail-capped delta `output`, `exit_code` on a finished command, facts, and `open_command`. `stalled` = unseen output stayed quiet for the stall window (a TUI question, a finished step) with read-it-and-act guidance. `no_activity` (active-hold daemons, v0.1.17+) = the visible screen stayed static for the daemon's 10s static cap — the wait only holds while the terminal is visibly changing; the note says the program looks idle: act instead of re-waiting (design/terminal-launch-proof-and-active-wait.md). Re-waiting after a stall holds while the terminal changes; against a static screen each re-wait costs one static cap. Budget expiry is a normal `timeout` result with call-again guidance plus a best-effort delta; a human interrupt is `interrupted`; a daemon without awaited observes yields an immediate snapshot with an upgrade note. `superseded` (the pending observe rejected with `superseded_by_user_message`) sets `result.superseded`, and the agent loop finishes the turn with `final.reason: "superseded_by_user_message"` without another provider call.
...
- human terminal interrupts reject the currently pending terminal wait as `interrupted`; `TerminalToolExecutor` turns that into a conservative tool result (`error: "interrupted"`, unresolved `still_running` command status for a submitted line at a shell) so the model regains control without treating the original command as completed.
...
- `terminal_send` tool calls parse into `terminal.send` directives (`text`, tolerating the legacy `raw_text` field); `terminal_run` is unknown and yields no directive
...
- `terminal.send`: dispatch `{ text, submit, await: "auto" }` (or `{ key, await: "auto" }`); on `command_finished` fetch the command's output slice via `TerminalSessionManager.getCommandOutput(command_id)` (tail-keeping 64 KiB cap) and return `kind:"command"` `{ exit_code, duration_ms, output, truncated?, mode, cwd? }`; on `interactive_started` return `status:"interactive"` with `ready`/`painted` plus the LAUNCH PROOF — a `captureDeltaProof` delta observe showing what the program painted (a trust/consent prompt, a login screen, its real input surface), attached so the model reads the screen before typing (debug/terminal-send-interactive-launch-blind-screen.md); `input_absorbed` attaches the same delta proof; otherwise observe the delta and return `kind:"interaction_ack"` with `gated_ms`/`program_ready`
- report a still-running result (with wait guidance) when the service wait budget expires on a submitted line at a shell — never a fabricated failure; dispatch failures map to structured `EXEC_FAILED`
...
**Current Coverage**:
- `terminal.send` command happy path: `await: "auto"` dispatch, exit 0, output slice, cwd/mode facts; launch → `interactive` with readiness facts; input into an open program carries `gated_ms`/`program_ready`
...
- `agent.tool_result` top-level convenience fields cover all terminal result kinds, including `terminal.wait` (`outcome`, `waited_ms`; `exit_code` falls back to the wait's command exit code) so live clients need not parse the persisted payload
...
- `/agent/state.pending_tool.args` and `agent.tool_call.args` are the model-facing tool args: `terminal.send { text, submit? | key }`, `terminal.observe { view?, lines? }`; there is no `wait_for` decoration — clients detect long terminal waits from the pending `terminal.send`/`terminal.wait` tool itself
...
## Click targets and observed link URLs — Phase 7

`browser-tools.ts` describes exact URLs, unnamed pointer hints and selection of the
specific observed control. Element handles use native Playwright clicks and offer
`geometry()` plus optional bounded `click({position:{x,y}})` when layout evidence
requires precision. No force, sibling substitution or automatic retry/navigation.
...
## Browser REPL Phase 4 output guidance

The REPL tool description documents 8 KiB default emitted text, explicit
`repl.setOutputBudget(bytes)` expansion up to 32 KiB, labeled overflow excerpts and
bounded artifact recall without repeating actions. Snapshot and visible-DOM reads
both return objects with `nodes` and replace action references. Examples preserve
coverage and distinguish samples from complete reads; verification targets the
changed element/page state rather than unrelated forms or application internals.
Local page coverage and historical tool replay are unchanged. Production catalog
cutover remains gated on comparison and lifecycle acceptance in the implementation
plan. See [validation](../../../debug/browser-repl-phase4.md).
...
## Browser REPL selective extraction — Phase 6 evaluation

The expanded opt-in comparison in `service/scripts/compare-browser-repl.ts`
compares saved baseline/candidate REPL catalogs on the same corrected Phase 5
runtime. Neutral fixtures verify exact attribution, coverage, retained evidence
and mutation counts, and report actual provider usage and output/recovery costs.
Expanded guidance and a narrower follow-up reuse cue were tested and reverted:
fewer browser reads did not establish lower context or fewer model calls.
The existing Phase 5 tool description remains unchanged by Phase 6.

See [Phase 6](../../../plan/bud-owned-browser/repl-phase-6-selective-extraction.md)
and [measurements](../../../debug/browser-repl-phase6.md). Context-efficiency
acceptance, live review and Phase 4 lifecycle/catalog-cutover gates remain open.
No new extractor, output budget, compaction or authority path is introduced.


## Compact snapshot discovery — Phase 7b

`browser-tools.ts` teaches `snapshot.format({nodes?,maxBytes?})` as a local compact
view, with short handles bound through `snapshot.getByReference("eN")` and exact
URL lookup through `snapshot.url("uN")`. Full nodes remain available for targeted
JS selection. Source coverage, view omissions and output truncation are distinct.
Overflow recovery selects retained data or bounded artifact excerpts without action
