...
| `terminal_wait` | none | Knobless: park the turn until the terminal needs attention (command finished, prompt returned, or unseen output went quiet — `stalled`), then return the delta; the model configures nothing |
