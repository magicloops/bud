import { settledTurnTiming } from "./invocation-timing.js";
import type { Invocation } from "./invocation-repository.js";

export function serializeInvocation(row: Invocation) {
  return { ...(settledTurnTiming(row) ?? {}), invocation_id: row.id, turn_id: row.turnId, input_message_id: row.inputMessageId,
