...
export const AUTOMATION_CANONICAL_TOOLS: CanonicalTool[] = [
  ...
  { name: "automations_create_draft", description: "Prepare an automation from the user's request without enabling it. Only name and instruction are required. Omitted settings use this exact Bud/model/reasoning, this current conversation, all permitted contact sources, Contacts-only access with at most 30 days of currently granted history, a 24-hour start deadline and 10 runs/day. Omit target (or use null) to keep results in this conversation; do not guess its ID. Use target mode new_thread only when the user explicitly requests a new conversation per trigger; preserve explicitly chosen existing conversations too. State the saved destination before requesting review. Preserve explicit user choices. Initial contact import does not trigger the rule. Request activation separately for human review; you cannot grant data permission." + guidance,
    parameters: { ...definition, required: ["name", "instruction"] } },
