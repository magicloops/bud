...
export const AUTOMATION_CANONICAL_TOOLS: CanonicalTool[] = [
  { name: "automations_list", description: "List automations targeting this conversation by default. Use scope all only for account-wide discovery or management requested by the user. Each result includes its saved destination. An empty thread list means this chat has no attached rules; create a separate rule for a new workflow." + guidance,
    parameters: { type: "object", additionalProperties: false, properties: { scope: { type: "string", enum: ["thread", "all"], description: "Omit or use null for this conversation; all lists this owner’s automations across conversations." } }, required: [] } },
