import { selectedBrowserTools, BROWSER_TOOL_NAMES } from "./browser-tools.js";
...
export function resolveAgentToolsForEnvironment(
  ...
  const tools = [...AGENT_CANONICAL_TOOLS,
    ...(options.browser ? selectedBrowserTools().filter(tool => options.browserHandoff || tool.name !== "browser_request_handoff") : []),
