...
- outbound `terminal_observe` carries `{ view, lines, await?, quiet_ms? }` (default view `screen`); `await` (`settled` | `command`, synonyms under the knobless wait) makes it an awaited observe; `quiet_ms` is the stall window (unseen-output-quiet duration that returns control) and rides along only with `await: "settled"`.
