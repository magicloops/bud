...
export class BudConnection {
  ...
  private async handleRaw(raw: string) {
    ...
    switch (envelope.data.type) {
      ...
      case "terminal_output":
        ...
        break;
      case "terminal_event":
        await this.handleTerminalEvent(parsed);
        break;
      case "terminal_grid":
        await this.handleTerminalGrid(parsed);
        break;
  ...
  private async handleTerminalEvent(raw: unknown) {
    if (!config.terminalEnabled || this.state.kind !== "connected") {
      return;
    }
    const result = TerminalEventSchema.safeParse(raw);
    if (!result.success) {
      logGatewayDebug({ error: result.error.message }, "Invalid terminal_event frame");
      return;
    }
  ...
  private async handleTerminalGrid(raw: unknown) {
    if (!config.terminalEnabled || this.state.kind !== "connected") {
      return;
    }
    const result = TerminalGridSchema.safeParse(raw);
    if (!result.success) {
      logGatewayDebug({ error: result.error.message }, "Invalid terminal_grid frame");
      return;
    }
