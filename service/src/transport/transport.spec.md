...
- selects carriers for stream families such as `file_read`, `localhost_http_proxy`, `localhost_websocket_proxy`, and `local_llm_http`
...
- counts active runtime streams per Bud/stream family for file/proxy/local-LLM concurrency enforcement; WebSocket proxy sessions register marker runtime streams so data-plane finalization can close active browser sockets on daemon disconnect
...
This adapter is the baseline carrier for Phase 0/1. Capable sessions now dispatch active terminal/control frames through typed protobuf oneof payload tags with direct payload fields. When a daemon advertises `bud_envelope.stream_frames`, the authenticated WebSocket is also registered as a control+data data-plane carrier for file/proxy/local-LLM stream frames, including the Phase 5 `localhost_websocket_proxy` family when the daemon advertises `proxy.localhost_websocket` and the `local_llm_http` family when the daemon advertises healthy Bud-local ds4 Responses capability.
