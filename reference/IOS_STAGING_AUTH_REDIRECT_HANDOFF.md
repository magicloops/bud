...
**Status:** Investigation complete; likely backend/web action required  
...
- callback scheme: `chat.bud.app`
- scopes: `openid profile email offline_access api`

Important implementation facts:

- the iOS app includes `redirect_uri=chat.bud.app://oauth/callback` in the authorize request
- the app starts `ASWebAuthenticationSession` with `callbackURLScheme = "chat.bud.app"`
...
- if the backend had emitted `chat.bud.app://oauth/callback?...`, the iOS auth session should have resumed
...
- its allowed redirect URIs include `chat.bud.app://oauth/callback` exactly
...
1. Inspect the staging iOS authorize request and confirm the server receives:
   ...
   - `response_type=code`
   - PKCE parameters
2. Confirm `bud-ios-staging` is registered with `chat.bud.app://oauth/callback` exactly.
...
5. Confirm the final redirect after successful hosted auth is a `302` to `chat.bud.app://oauth/callback?...`, not a navigation to `https://staging.bud.dev`.
...
5. The final redirect target is `chat.bud.app://oauth/callback?code=<code>&state=<state>`.
...
2. Log the final redirect target after provider auth on staging.
   ...
   - If it is `chat.bud.app://oauth/callback?...`, then iOS investigation should continue.

## Requested Backend Response

Please respond with:

1. confirmation that `bud-ios-staging` exists and is registered with `chat.bud.app://oauth/callback`
...
5. whether staging should continue using `chat.bud.app://oauth/callback` or switch to a distinct staging callback URI
