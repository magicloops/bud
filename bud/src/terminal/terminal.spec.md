...
Owns thread terminal sessions on the Bud host. The module creates or reattaches tmux sessions, pipes output to the service, dispatches interactive input, captures screen/readiness state, and exposes narrow backend utilities needed by adjacent daemon adapters.
...
Terminal send/input handling, output waits, and request-response result construction.

### `observe.rs`

Explicit observe request handling and screen/history capture.
