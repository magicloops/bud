...
- `addon.rs`: opt-in browser add-on (Phase 3r): `manifest.json` under
  ...
  keyed by SHA-256 of its archive (including dependencies), rejection of mismatched
  managed helper identities with a prepare instruction,
  the launch probe and observed-version drift recording. Unit-tested without
  Chrome or network.
...
only reads it: `BrowserManager::configured_for` resolves the runtime, probes a
launch (browser, Node and helper all exercised), requires the version floor
...
refreshed; nothing else is written by the daemon. The helper is packed by
