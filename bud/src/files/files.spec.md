...
This folder owns the daemon's local adapter for service-requested file stat/read/range streams. It revalidates service `file_open` requests against daemon-side policy before touching the filesystem, tries the thread terminal pane cwd before the configured workspace root when terminal context is present, reads only regular files under the configured workspace root, and streams accepted bytes over the active data-plane carrier with runtime credits.
...
- tries `pane_current_path + relative_path` first when the pane cwd is inside the workspace root, then falls back to `workspace_root + relative_path` only when the cwd candidate is missing
