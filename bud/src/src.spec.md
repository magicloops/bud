...
- **`handle_send`** - Structured interactive input path for agent `terminal.send`:
  ...
  - Deliberately waits `10ms` before a `submit_flag` Enter that immediately follows literal text; this is an intentional tmux-backed TUI compatibility boundary for Codex and Claude Code, not a temporary debugging delay
