...
| [PR_SUMMARY.md](./PR_SUMMARY.md) | Short root-level PR handoff summarizing the current false Bud-offline hypothesis test, the backend session-ownership guardrails that landed, the staging validation matrix, and the fallback investigation path if the bug persists |
