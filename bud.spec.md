...
| [reference/IOS_AGENT_STREAM_STATE_AND_RESUME_HANDOFF.md](./reference/IOS_AGENT_STREAM_STATE_AND_RESUME_HANDOFF.md) | Draft proposed backend handoff for the next agent-stream contract, introducing `/agent/state`, opaque resume cursors, bounded catch-up, and explicit `resync_required` recovery for mobile reconnects |
| [reference/IOS_AGENT_STREAM_STATE_AND_RESUME_FIXTURES.md](./reference/IOS_AGENT_STREAM_STATE_AND_RESUME_FIXTURES.md) | Draft proposed fixtures for the next agent-stream contract, covering passive open, active-turn bootstrap, bounded cursor resume, explicit resync, and idle-to-active cursor races |
