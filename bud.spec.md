...
| [AGENTS.template.md](./AGENTS.template.md) | Portable, project-agnostic `AGENTS.md` starter that preserves the repo's intent-doc-first and spec-maintenance workflow for reuse in other repositories |
...
| [plan/portable-agents-template.md](./plan/portable-agents-template.md) | Short plan note describing how the Bud-specific operating guide was generalized into a reusable `AGENTS.md` template without carrying over stack- or domain-specific assumptions |
...
| [plan/thread-terminal-boundaries/implementation-spec.md](./plan/thread-terminal-boundaries/implementation-spec.md) | Phased implementation plan for fixing the thread-view terminal `1;2c` bug class structurally by separating human input from emulator protocol, moving browser terminal input onto an explicit transport boundary, replacing raw-history bootstrap with safe terminal state plus explicit durable resume, and adding a follow-up shared-send phase for optional observation |
| [plan/thread-terminal-boundaries/phase-6-optional-observation-send-adoption.md](./plan/thread-terminal-boundaries/phase-6-optional-observation-send-adoption.md) | Follow-up phase for restoring low-latency browser typing by making `terminal_send.observe` optional, keeping one shared daemon/runtime send implementation, and updating xterm/browser plus agent usage to request observation only when needed |
| [plan/thread-terminal-boundaries/phase-7-rich-bootstrap-contract-and-capture-metadata.md](./plan/thread-terminal-boundaries/phase-7-rich-bootstrap-contract-and-capture-metadata.md) | Follow-up phase for evolving `/terminal/state` into an explicit richer bootstrap contract with visible-screen grid, cursor, geometry, and screen-mode metadata plus degraded fallback modes |
| [plan/thread-terminal-boundaries/phase-8-browser-rich-bootstrap-adoption-and-cursor-fidelity.md](./plan/thread-terminal-boundaries/phase-8-browser-rich-bootstrap-adoption-and-cursor-fidelity.md) | Follow-up phase for adopting richer bootstrap kinds in the reference web client, restoring cursor placement more faithfully on fresh page opens, and removing or scoping the temporary blank-line trim workaround |
| [plan/thread-terminal-boundaries/phase-9-rollout-cleanup-and-contract-tightening.md](./plan/thread-terminal-boundaries/phase-9-rollout-cleanup-and-contract-tightening.md) | Follow-up cleanup phase for tightening the internal-only terminal-boundary contract by removing temporary compatibility fields, transitional bootstrap surfaces, and validation-era debug scaffolding once the new contract is validated |
| [plan/thread-terminal-boundaries/phase-10-emulator-protocol-suppression-and-raw-input-narrowing.md](./plan/thread-terminal-boundaries/phase-10-emulator-protocol-suppression-and-raw-input-narrowing.md) | Follow-up live-input phase for suppressing or narrowly allowlisting xterm-generated emulator replies, narrowing `/terminal/input` away from emulator-protocol traffic, and expanding structured browser key coverage for the remaining human fallback cases |
| [plan/thread-terminal-boundaries/validation-checklist.md](./plan/thread-terminal-boundaries/validation-checklist.md) | Validation checklist for the thread-terminal-boundaries work, covering browser source classification, structured browser input, safe terminal bootstrap, live-only stream attach, durable offset resume, and docs/spec alignment |
| [plan/thread-terminal-cursor-bootstrap-validation.md](./plan/thread-terminal-cursor-bootstrap-validation.md) | Focused validation plan for the post-boundaries xterm cursor-regression, mapping the current safe-bootstrap hypotheses to temporary service/web instrumentation before any behavioral fix |
...
| [debug/thread-terminal-codex-tui-osc-color-reply-leak.md](./debug/thread-terminal-codex-tui-osc-color-reply-leak.md) | Debug note documenting the Codex TUI startup symptom where an apparent OSC 10 / 11 color-query reply leaks into the input area, reviewing the current browser raw-emulator path and ranking the likely causes around terminal-reply forwarding through tmux |
| [debug/terminal-send-codex-submit-enter-regression.md](./debug/terminal-send-codex-submit-enter-regression.md) | Debug note reviewing the current shared `terminal.send` path for a Codex-vs-Claude submit regression, with the main hypotheses around Codex-specific submit semantics, accidental double-Enter from trailing newlines plus `submit:true`, and the weaker but relevant gap that Codex is not yet a known interactive program |
...
| [design/terminal-human-input-boundaries-and-replay-semantics.md](./design/terminal-human-input-boundaries-and-replay-semantics.md) | Design for fixing the thread-view `1;2c` class of bugs structurally by separating human input from emulator protocol, moving browser input onto an explicit transport boundary, and giving terminal bootstrap/reconnect the same live-only-plus-explicit-resume semantics we now prefer for agent streams |
| [design/browser-terminal-typing-latency-and-send-modes.md](./design/browser-terminal-typing-latency-and-send-modes.md) | Design review of the new browser terminal typing latency, comparing reverting to raw `/terminal/input` versus forward options that keep browser input in the `terminal_send` family, then ranking them by simplicity and long-term robustness with an explicit recommendation |
| [design/daemon-terminal-send-ack-and-optional-observation.md](./design/daemon-terminal-send-ack-and-optional-observation.md) | Daemon-focused design review of keeping one `terminal_send` implementation while splitting dispatch acknowledgment from optional observation, including the current `main.rs` unknowns around frame ordering, baseline capture, shared delivered-capture state, and dispatch semantics |
| [design/terminal-rich-bootstrap-contract.md](./design/terminal-rich-bootstrap-contract.md) | Design for evolving `/terminal/state` from a text-only safe snapshot into an explicit bootstrap contract with visible-screen grid data, cursor/geometry metadata, degraded fallback modes, and optional pathways for geometry mismatch and browser hydration |
