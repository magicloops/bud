...
Bud availability is treated as an agent environment. When a thread's Bud is offline at send time, the message route still starts an assistant turn, exposes `bud_offline` through `/agent/state.environment` and the send response, injects request-time offline guidance, and removes only Bud-specific terminal/web-view tools from the provider catalog. Non-Bud service tools, including `ask_user_questions`, remain available by default, and active turns refresh the environment before provider/tool steps so a reconnect can restore Bud tools mid-turn.
...
| [plan/personal-data-ingestion-and-agent-triggers/personal-data-ingestion-and-agent-triggers.spec.md](./plan/personal-data-ingestion-and-agent-triggers/personal-data-ingestion-and-agent-triggers.spec.md) | Implementation plan directory index, phase dependencies, progress and validation references |
...
| [reference/IOS_AGENT_STREAM_RESYNC_RACE_HANDOFF.md](./reference/IOS_AGENT_STREAM_RESYNC_RACE_HANDOFF.md) | Focused iOS handoff for the observed mobile `agent.resync_required` race, where a known-invalid cursor is retried while `/agent/state` recovery is still in flight, plus the proposed serialized stream-recovery state machine |
| [reference/IOS_AGENT_MESSAGE_DURATION_METADATA_HANDOFF.md](./reference/IOS_AGENT_MESSAGE_DURATION_METADATA_HANDOFF.md) | Backend handoff for the per-message agent work-duration metadata contract, covering durable timing fields, live assistant timing, mobile grouping guidance, validation status, and deferred turn-level duration gaps |
...
| [plan/ask-question-tool/ask-question-tool.spec.md](./plan/ask-question-tool/ask-question-tool.spec.md) | Folder spec for the phased `ask_user_questions` rollout across service contracts, agent runtime, reference web UI, human-input notifications, and validation |
| [plan/ask-question-tool/implementation-spec.md](./plan/ask-question-tool/implementation-spec.md) | Parent implementation spec for adding the `ask_user_questions` model tool, durable question requests, owner-scoped responses, live tool-result continuation, restart fallback, and web form rendering |
| [plan/ask-question-tool/progress-checklist.md](./plan/ask-question-tool/progress-checklist.md) | Running checklist for the ask-user-questions service and web implementation |
| [plan/ask-question-tool/validation-checklist.md](./plan/ask-question-tool/validation-checklist.md) | Manual validation checklist for question request validation, ownership, live continuation, restart fallback, web rendering, and notification behavior |
| [plan/fix-drizzle-push-mismatch/fix-drizzle-push-mismatch.spec.md](./plan/fix-drizzle-push-mismatch/fix-drizzle-push-mismatch.spec.md) | Folder spec for the Drizzle/PostgreSQL push-convergence cleanup plan |
| [plan/fix-drizzle-push-mismatch/implementation-spec.md](./plan/fix-drizzle-push-mismatch/implementation-spec.md) | Phased implementation spec for normalizing overlong FK names, one-column primary-key metadata, checked-in migrations, and `db:push` convergence validation |
| [plan/fix-drizzle-push-mismatch/validation-checklist.md](./plan/fix-drizzle-push-mismatch/validation-checklist.md) | Validation checklist for the Drizzle push mismatch fix, including metadata guardrails, catalog checks, and repeated `db:push` convergence |
...
| [plan/message-duration-metadata/message-duration-metadata.spec.md](./plan/message-duration-metadata/message-duration-metadata.spec.md) | Folder spec for the per-message duration metadata rollout, extending tool timing to reasoning and assistant artifacts without a turn table |
| [plan/message-duration-metadata/implementation-spec.md](./plan/message-duration-metadata/implementation-spec.md) | Parent implementation spec for standardizing `started_at`, `finished_at`, `duration_ms`, and `duration_source` across agent-created message metadata |
| [plan/message-duration-metadata/phase-1-contract-and-helper-foundation.md](./plan/message-duration-metadata/phase-1-contract-and-helper-foundation.md) | Foundation phase covering shared timing helpers, source labels, metadata serialization, and helper tests |
| [plan/message-duration-metadata/phase-2-tool-and-reasoning-metadata.md](./plan/message-duration-metadata/phase-2-tool-and-reasoning-metadata.md) | Persistence phase covering durable tool `turn_id`, tool `duration_source`, and reasoning `duration_ms` / `duration_source` metadata |
| [plan/message-duration-metadata/phase-3-assistant-timing-stream-and-persistence.md](./plan/message-duration-metadata/phase-3-assistant-timing-stream-and-persistence.md) | Assistant phase covering draft start tracking, live assistant timing events, and persisted assistant timing metadata |
| [plan/message-duration-metadata/phase-4-client-types-docs-and-validation.md](./plan/message-duration-metadata/phase-4-client-types-docs-and-validation.md) | Finalization phase covering protocol docs, service specs, web type alignment, mobile fixtures, and validation |
| [plan/message-duration-metadata/progress-checklist.md](./plan/message-duration-metadata/progress-checklist.md) | Running implementation checklist for the message-duration metadata rollout |
| [plan/message-duration-metadata/validation-checklist.md](./plan/message-duration-metadata/validation-checklist.md) | Manual and automated validation checklist for per-message duration metadata across live stream, runtime state, and persisted history |
...
| [plan/fix-send-keys-parse/fix-send-keys-parse.spec.md](./plan/fix-send-keys-parse/fix-send-keys-parse.spec.md) | Folder spec for the narrow daemon `tmux send-keys` parse fix, scoped to literal text that begins with option-shaped content such as markdown bullets |
| [plan/fix-send-keys-parse/implementation-spec.md](./plan/fix-send-keys-parse/implementation-spec.md) | Implementation plan for adding a tmux option terminator to daemon literal-text dispatch and covering leading-dash regression cases |
| [plan/fix-send-keys-parse/validation-checklist.md](./plan/fix-send-keys-parse/validation-checklist.md) | Validation checklist for leading-dash markdown sends, real tmux smoke coverage, and no-change service/protocol checks |
...
| [debug/staging-route-error-api-me-404-commit-review.md](./debug/staging-route-error-api-me-404-commit-review.md) | Debug note reviewing the current staging route error and local `/api/me/` 404 against the `1a64eac..HEAD` commit range, with follow-up findings around a Chrome-cached `308` from `/api/me` to `/api/me/`, local API topology drift, staging API front-door routing, and migration skew |
...
| [debug/offline-bud-message-persisted-without-agent-turn.md](./debug/offline-bud-message-persisted-without-agent-turn.md) | Debug note reviewing the offline-Bud message-send edge case where `POST /messages` persists a user row before agent startup proves terminal availability, then returns `bud_offline` without an agent `final` event |
...
| [render.yaml](./render.yaml) | Render Blueprint for the prototype staging deployment, declaring the separate `bud-web`, `bud-service`, and `bud-postgres` resources along with monorepo build boundaries (the web build fetches tags first so its baked build tag resolves), auth/env placeholders, and hosted web-view proxy env placeholders |
