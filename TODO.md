...
- [ ] HealthKit durability, additional samples, source precedence, lookback/backlog and summary windows; retain the existing mobile producers until this follow-up.
- [ ] Geographic regions, named places and more timely arrival triggers; current location remains best-effort evidence with uncertainty and no repeated completed actions.
- [ ] Viewer-authenticated app-data broker; development apps currently use explicitly approved owner-scoped backend keys.
- [ ] Dedicated revocable upload-only credentials if background transfer delays outlast bearer validity.
- [ ] Decide shipped sign-out queue disposal (likely discard); retain account/environment-isolated queues during development for testing.
- [ ] Production retention/deletion/backup periods and encryption/key-management design.
- [ ] Revisit contact change-history optimization, additional subscription categories and photos only in a separately scoped change; no Apple Contacts write-back in this pass.

## Immediate

- [ ] **Persistent browser REPL.** Phases 1–3 implemented and default to REPL outside production (`BUD_BROWSER_TOOL_MODE=tools` for comparison); Phase 2 real-provider observation/navigation acceptance passed. Next: real-agent interaction and web/iPhone handoff acceptance ([validation](debug/browser-repl-interactions.md)), then comparison/cutover in the [four-phase plan](./plan/bud-owned-browser/repl-implementation.md): worker/control foundations, selective observations through the actual agent, full interactions/lifecycle acceptance, then measured coordinated tool cutover. Preserve memory on normal takeover; keep large intermediate observations local and emit only selected evidence. [Design](./design/browser-repl.md).
...
- [ ] **Browser support as a daemon add-on.** Implemented locally (2026-09-21); remaining: release-build and Ubuntu acceptance. [Phase 3r](./plan/bud-owned-browser/phase-3r-browser-addon.md) from [design/browser-addon.md](./design/browser-addon.md): manifest under the base dir, system-browser detection first, managed Chromium only as plan B, managed Node/helper released with the daemon, `bud browser prepare/status/remove`, and `doctor` parity. Removes the compile-time helper path and the `BUD_BROWSER_EXECUTABLE` requirement. Linux secure-storage/sandbox caveats stay with Phase 3o.
